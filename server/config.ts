...
export const config = {
  ...
  ai: {
    ...
    liveModel: process.env.GEMINI_LIVE_MODEL || 'gemini-3.5-transcribe-live',
    flashModel: process.env.GEMINI_MODEL || 'gemini-3.5-flash',
    proModel: process.env.GEMINI_PRO_MODEL || 'gemini-3.5-pro',
    fallbackModels: (process.env.GEMINI_FALLBACK_MODELS || 'gemini-3.5-flash,gemini-2.5-flash,gemini-2.0-flash').split(',').map(m => m.trim()),
